 ...
 *  changed by dhx 2016 to allow the update of the Book value without touching
 *  the count value. 
 *	
 *  Original:
 *  @author Jorg Janke
 *  @version $Id: InventoryBookingCountUpdate.java,v 1.2 2006/07/30 00:51:01 jjanke Exp $
 */
public class InventoryBookingCountUpdate extends SvrProcess
{
	/** Physical Inventory		*/
	private int		p_M_Inventory_ID = 0;
	/** Update to What			*/
	private String	p_InventoryCountSet = "";
	...
	protected void prepare()
	{
		...
		for (int i = 0; i < para.length; i++)
		{
			...
			if (para[i].getParameter() == null)
				;
			else if (name.equals("InventoryCountSetOptional"))
				p_InventoryCountSet = (String) para[i].getParameter();
			else
	...
	protected String doIt () throws Exception
	{
		...
		sql = new StringBuilder("UPDATE M_InventoryLine l ")
			.append("SET QtyBook = ")
				...
				.append(" GROUP BY s.M_Product_ID,s.M_Locator_ID,s.M_AttributeSetInstance_ID),")
		...
		//	Set Count to Zero
		if ("Z".equals(p_InventoryCountSet))
		{
			sql = new StringBuilder("UPDATE M_InventoryLine l ")
		...
		} else if ("O".equals(p_InventoryCountSet)) {
			sql = new StringBuilder("UPDATE M_InventoryLine l ")
				.append("SET QtyCount=QtyBook ")
				.append("WHERE M_Inventory_ID=").append(p_M_Inventory_ID)
				.append(" AND EXISTS (SELECT * FROM M_StorageOnHand s ")
				.append("WHERE s.M_Product_ID=l.M_Product_ID AND s.M_Locator_ID=l.M_Locator_ID")
				.append(" AND s.M_AttributeSetInstance_ID=l.M_AttributeSetInstance_ID)");
			no = DB.executeUpdate(sql.toString(), get_TrxName());
			if (log.isLoggable(Level.INFO)) log.info("Set count to QtyBook=" + no);
		} else if ("N".equals(p_InventoryCountSet)) {
			if (log.isLoggable(Level.INFO)) log.info("Not updating count of inventory lines.");			
		} else {
			throw new AdempiereSystemError ("Invalid InventoryCountSet parameter value: \"" + p_InventoryCountSet + "\"");
		}
	...
	private int updateWithMA()
	{
		...
		try
		{
			...
			while (rs.next ())
			{
				...
				if ("O".equals(p_InventoryCountSet)) {
